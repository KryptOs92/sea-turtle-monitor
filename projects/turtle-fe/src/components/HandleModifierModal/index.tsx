...
import { addModifier } from "../../methods";
...
interface ModifierModalProps {
  ...
  modifyModifier: (addr: string) => Promise<void> | void;
  buttonLabel?: string; // testo opzionale sul pulsante
}

export default function ModifierModal({ mode, initialAddress = "", modifyModifier, buttonLabel }: ModifierModalProps) {
  ...
  const handleConfirm = async () => {
    if (mode === "create") {
      if (address && address.length > 0) {
        let result = null;
        setShowLoader(true);
        result = await addModifier(turtleClient, address);
        if (!result.success) {
  ...
  const title = mode === "create" ? "Aggiungi modifier" : "Modifica modifier";
