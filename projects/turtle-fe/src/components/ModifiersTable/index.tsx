...
function EnhancedTableToolbar(props: EnhancedTableToolbarProps) {
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        <Typography sx={{ flex: "1 1 100%" }} color="inherit" variant="caption" component="div">
...
export default function ModifiersTable({ modifiers = [] }): React.FC<ModifiersTableProps> {
  ...
  return (
    <>
      <EnhancedTableToolbar itemsSelected={selected} numSelected={selected.length} />
      <TableContainer>
        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle" size={dense ? "small" : "medium"}>
          ...
          <TableBody>
            {visibleRows.map((row, index) => {
              const isItemSelected = selected.includes(row.address);
