...
export const turtleSCslice = createSlice({
  ...
  reducers: {
    ...
    setTurtleClient: (state, action: PayloadAction<any>) => {
      state.turtleClient = action.payload;
    },
  },
});

// Esporta le azioni generate da createSlice
export const { setUserAuthorityScCreator, setUserAuthorityTurtleCreator, setUserAuthorityScModifier, setTurtleClient } =
  turtleSCslice.actions;
